  ...
  return (
    <>
      <style>{`
        ...
        /* SVG card: fixed height so graph never clips */
        .gp-svg-card {
          background: var(--bg2);
        ...
        /* Info panel: 2-col grid on mobile, sidebar on desktop */
        .gp-info {
          display: grid;
      ...
      <div className="gp-page">
              ...
              {NODES.map(n => (
                <g key={n.id}>
                  <circle cx={n.x} cy={n.y} r={22}
                    fill={nodeColor(n.id)} stroke={nodeStroke(n.id)} strokeWidth={2}
          ...
          <div className="gp-btns">
            ...
            {activeAlgo && running && (
              <span style={{
                ...
                borderRadius: 4, color: '#a78bfa', fontFamily: 'JetBrains Mono',
              }}>
                {activeAlgo.toUpperCase()} Running...
              </span>
            )}
          </div>
        </div>
        ...
        <div className="gp-info">
          ...
          <div className="gp-info-full" style={{
            background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: 14,
          }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text2)', letterSpacing: '0.08em', marginBottom: 10 }}>
              COMPLEXITY
            </div>
            {[
              ['Time',      'O(V + E)', '#22d3a0'],
              ['Space BFS', 'O(V)',     '#38bdf8'],
              ['Space DFS', 'O(V)',     '#a78bfa'],
              ['Vertices',  NODES.length, 'var(--text)'],
              ['Edges',     EDGES.length, 'var(--text)'],
            ].map(([k, v, c]) => (
              <div key={k} style={{
          ...
          <div className="gp-info-full" style={{
            ...
            <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text2)', letterSpacing: '0.08em', marginBottom: 8 }}>
              BFS vs DFS
            </div>
            <div style={{ fontSize: 11, color: 'var(--text2)', fontFamily: 'JetBrains Mono', lineHeight: 1.8 }}>
              <b style={{ color: '#38bdf8' }}>BFS:</b> Uses a Queue, explores level by level.
              Best for shortest path.<br /><br />
              <b style={{ color: '#22d3a0' }}>DFS:</b> Uses a Stack, explores depth first.
              Best for cycle detection and topological sort.
            </div>
          </div>
        </div>
...
function InfoBox({ title, color, children }) {
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: 14 }}>
      <div style={{ fontSize: 10, fontWeight: 700, color, letterSpacing: '0.08em', marginBottom: 8 }}>
        {title.toUpperCase()}
      </div>
      {children}
    </div>
